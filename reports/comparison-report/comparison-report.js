...
class ComparisonReport {
    ...
    _createQASummarySheet(wb, at, rt, deltas, meta) {
        ...
        // Live-only detection: when no Auto Test KPIs exist (every AT
        // value is missing/non-numeric) the comparison is meaningless
        // and the legacy PARITY-PASS verdict was misleading. Render
        // an INFORMATIONAL banner instead so the QA reader knows the
        // workbook is a live-session report with no AT reference.
        const atIsEmpty = !KPI_FIELDS.some(f => typeof at[f.key] === 'number');
        const rtHasData = KPI_FIELDS.some(f => typeof rt[f.key] === 'number');
        if (atIsEmpty && rtHasData) {
            verdict.value = 'ℹ INFORMATIONAL — Live session, no Auto Test reference (no parity comparison performed)';
            verdict.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFFF3CD' } };
            verdict.font = { bold: true, size: 13, color: { argb: 'FF856404' } };
    ...
    _createAutoTestSpinsSheet(wb, at) {
        ...
        const headers = ['#', 'Action', 'Spin#', 'Next#', 'Pair', 'Filter', 'Predicted Numbers', 'Bet/Num', 'Hit', 'P&L', 'Bankroll'];
        const headerRow = sheet.getRow(1);
        headers.forEach((h, i) => {
            const cell = headerRow.getCell(i + 1);
            cell.value = h;
        ...
        steps.forEach((s, i) => {
            const row = sheet.getRow(i + 2);
            // Full predicted-numbers list per row — same as the
            // Auto Test report sheet, so the user can verify each
            // BET decision row-by-row across both workbooks.
            const predictedList = (s && s.action === 'BET' && Array.isArray(s.predictedNumbers) && s.predictedNumbers.length > 0)
                ? s.predictedNumbers.slice().sort((a, b) => a - b).join(', ')
                : '--';
            const values = [
                i + 1,
                s.action || '--',
        ...
        sheet.columns = [
            ...
            { width: 16 }, { width: 16 }, { width: 40 }, { width: 10 },
    ...
    _createResultSpinsSheet(wb, rt) {
        ...
        const headers = ['#', 'Spin#', 'Bet Amount', 'Total Bet', 'Hit', 'Actual #', 'Predicted Numbers', 'Net Change', 'Timestamp'];
        const headerRow = sheet.getRow(1);
        headers.forEach((h, i) => {
            const cell = headerRow.getCell(i + 1);
            cell.value = h;
        ...
        hist.forEach((b, i) => {
            const row = sheet.getRow(i + 2);
            // Full predicted-numbers list — populated from
            // window.moneyPanel.pendingBet at bet-record time and
            // captured in the live betHistory entry. Falls back to
            // '--' when the live capture didn't carry the list.
            const predictedList = (b && Array.isArray(b.predictedNumbers) && b.predictedNumbers.length > 0)
                ? b.predictedNumbers.slice().sort((a, x) => a - x).join(', ')
                : '--';
            const values = [
                i + 1,
                b && b.spin != null ? b.spin : '--',
        ...
        sheet.columns = [
            { width: 6 }, { width: 8 }, { width: 12 }, { width: 12 },
            { width: 8 }, { width: 10 }, { width: 40 }, { width: 14 },
            { width: 26 }
        ];
    }
