// TABLE2 LOGIC  
// Uses lookup table with position codes: S+0, SL+1, SL-1, SR+1, SR-1, SR+2, SR-2, SL+2, SL-2, OR+1, OR-1, OL+1, OL-1, O+0, OR+2, OR-2, OL+2, OL-2
// Works with P, P-13OPP pairs (and P+1, P-1, P+2, P-2 pairs)

function calculateTable2Projections(refs, digit13Opp, prevPosCode) {
    if (prevPosCode === 'XX') return null;
    
    // Valid position codes for Table2 (extended set)
    const validCodes = [
        'S+0', 'SL+1', 'SL-1', 'SR+1', 'SR-1', 'SR+2', 'SR-2', 'SL+2', 'SL-2',
        'OR+1', 'OR-1', 'OL+1', 'OL-1', 'O+0', 'OR+2', 'OR-2', 'OL+2', 'OL-2'
    ];
...
function renderTable2Row(spin, idx, spins, refs, digit13Opp, calculatePositionCode) {
    ...
    ['prev', 'prev_plus_1', 'prev_minus_1', 'prev_plus_2', 'prev_minus_2'].forEach(refKey => {
        ...
        // Calculate Table2 projections
        const table2Proj = calculateTable2Projections(refs, digit13Opp, usePosCode);
        
        if (table2Proj && table2Proj[refKey]) {
            projections[refKey] = {
                p: table2Proj[refKey].p,
                p13: table2Proj[refKey].p13,
                posCode: usePosCode
            };
        }
    });
