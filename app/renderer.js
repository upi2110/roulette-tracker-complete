const WHEEL_STANDARD = [0,32,15,19,4,21,2,25,17,34,6,27,13,36,11,30,8,23,10,5,24,16,33,1,20,14,31,9,22,18,29,7,28,12,35,3,26];
        ...
        function getNumberAtPosition(refNum, posCode) {
            ...
            const refIdx = WHEEL_NO_ZERO.indexOf(ref);
            ...
            const actualDistance = sign === '+' ? distance : -distance;
            
            let targetIdx;
            
            if (side === 'S') {
                if (direction === 'L') {
                    targetIdx = (refIdx - actualDistance + 37) % 37;
                } else {
                    targetIdx = (refIdx + actualDistance) % 37;
                }
            } else {
                const oppNum = REGULAR_OPPOSITES[refNum];
                const opp = oppNum === 0 ? 26 : oppNum;
                const oppIdx = WHEEL_NO_ZERO.indexOf(opp);
                
                if (direction === 'L') {
                    targetIdx = (oppIdx - actualDistance + 37) % 37;
                } else {
                    targetIdx = (oppIdx + actualDistance) % 37;
                }
            }
            
            const resultNum = WHEEL_NO_ZERO[targetIdx];
        ...
        function render() {
            ...
            spins.forEach((spin, idx) => {
                ...
                if (prev === null) {
                    ...
                    const projHtml = (key) => {
                        ...
                        return purpleHtml + (p.green.length > 0 ? ' ' + greenHtml : '');
                    };
                    ...
                    row.innerHTML = `
                        ...
                        <td><strong>${spin.actual}</strong></td>
                        <td>${data.prev.ref}</td>
                        <td>${formatPos(data.prev.pair)}</td>
                        <td>${data.prev.ref13Opp}</td>
                        <td>${formatPos(data.prev.pair13Opp)}</td>
                        <td class="${projClass('prev')}">${projHtml('prev')}</td>
                        <td>${data.prev_plus_1.ref}</td>
                        <td>${formatPos(data.prev_plus_1.pair)}</td>
                        <td>${data.prev_plus_1.ref13Opp}</td>
                        <td>${formatPos(data.prev_plus_1.pair13Opp)}</td>
                        <td class="${projClass('prev_plus_1')}">${projHtml('prev_plus_1')}</td>
                        <td>${data.prev_minus_1.ref}</td>
                        <td>${formatPos(data.prev_minus_1.pair)}</td>
                        <td>${data.prev_minus_1.ref13Opp}</td>
                        <td>${formatPos(data.prev_minus_1.pair13Opp)}</td>
                        <td class="${projClass('prev_minus_1')}">${projHtml('prev_minus_1')}</td>
                        <td>${data.prev_plus_2.ref}</td>
                        <td>${formatPos(data.prev_plus_2.pair)}</td>
                        <td>${data.prev_plus_2.ref13Opp}</td>
                        <td>${formatPos(data.prev_plus_2.pair13Opp)}</td>
                        <td class="${projClass('prev_plus_2')}">${projHtml('prev_plus_2')}</td>
                        <td>${data.prev_minus_2.ref}</td>
                        <td>${formatPos(data.prev_minus_2.pair)}</td>
                        <td>${data.prev_minus_2.ref13Opp}</td>
                        <td>${formatPos(data.prev_minus_2.pair13Opp)}</td>
                        <td class="${projClass('prev_minus_2')}">${projHtml('prev_minus_2')}</td>
                        <td>${data.prev_prev.ref}</td>
                        <td>${formatPos(data.prev_prev.pair)}</td>
                        <td>${data.prev_prev.ref13Opp}</td>
                        <td>${formatPos(data.prev_prev.pair13Opp)}</td>
            ...
            if (spins.length > 0) {
                ...
                nextRow.innerHTML = `
                    ...
                    <td>${nextRefs.prev}</td>
                    <td></td>
                    <td>${DIGIT_13_OPPOSITES[nextRefs.prev]}</td>
                    <td></td>
                    <td class="col-prj">${projHtmlNext('prev')}</td>
                    <td>${nextRefs.prev_plus_1}</td>
                    <td></td>
                    <td>${DIGIT_13_OPPOSITES[nextRefs.prev_plus_1]}</td>
                    <td></td>
                    <td class="col-prj">${projHtmlNext('prev_plus_1')}</td>
                    <td>${nextRefs.prev_minus_1}</td>
                    <td></td>
                    <td>${DIGIT_13_OPPOSITES[nextRefs.prev_minus_1]}</td>
                    <td></td>
                    <td class="col-prj">${projHtmlNext('prev_minus_1')}</td>
                    <td>${nextRefs.prev_plus_2}</td>
                    <td></td>
                    <td>${DIGIT_13_OPPOSITES[nextRefs.prev_plus_2]}</td>
                    <td></td>
                    <td class="col-prj">${projHtmlNext('prev_plus_2')}</td>
                    <td>${nextRefs.prev_minus_2}</td>
                    <td></td>
                    <td>${DIGIT_13_OPPOSITES[nextRefs.prev_minus_2]}</td>
                    <td></td>
                    <td class="col-prj">${projHtmlNext('prev_minus_2')}</td>
                    <td>${nextRefs.prev_prev}</td>
                    <td></td>
                    <td>${DIGIT_13_OPPOSITES[nextRefs.prev_prev]}</td>
                    <td></td>
                    <td class="col-prj">${projHtmlNext('prev_prev')}</td>
                `;
